...
def parse_xml(filename):
    ...
    if datetime.datetime.fromtimestamp(os.path.getctime(filename)) > datetime.datetime(2018, 12, 7):
...
def yesterday(string=True):
    # Get formated yesterday. Param string: True to return string otherwise DateTime
    # Returns date as string or DateTime
...
def get_date_interval(date):
    # Get date internal. date internal as string, eg. 20150311:20150327. Returns list of dates
    ...
    if date.startswith(':'):
        ...
        date_start, date_end = date.split(':', 1)
