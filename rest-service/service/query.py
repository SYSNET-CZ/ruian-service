...
AP_COLUMNS_GET_LIST = "gid, cislo_domovni, cislo_orientacni, " \
                      "znak_cisla_orientacniho, psc, typ_so, " \
                      "'kod_ulice', kod_casti_obce, 'momckod', " \
                      "nazev_casti_obce, kod_obce, nazev_obce, nazev_ulice, nazev_momc, " \
                      "'mopkod', nazev_mop"
...
def _get_adresa(y, x):
    ...
    sql = "SELECT {0} {1} FROM {2} WHERE {3} ORDER BY {4} LIMIT 1;".format(
        dist, AP_COLUMNS_GET_LIST, 'address_points', within, 'd1')
